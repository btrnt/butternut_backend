...
class CTRL_LM(AbstractLanguageChecker):
    def __init__(self,  model_name_or_path="ctrl"):
        super(CTRL_LM, self).__init__()
        # self.enc = GPT2Tokenizer.from_pretrained(model_name_or_path)
        # self.model = GPT2LMHeadModel.from_pretrained(model_name_or_path)
        self.enc = transformers.CTRLTokenizer.from_pretrained(model_name_or_path)
        self.model = transformers.CTRLLMHeadModel.from_pretrained(model_name_or_path)
        self.start_token = '.'
    ...
    def postprocess(self, token):
        ...
        token = '-' if token.strtswith('â') else token
